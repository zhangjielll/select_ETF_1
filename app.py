...
# -*- coding: utf-8 -*-
"""
Streamlit ETF Scanner - 大A改良版CAN SLIM ETF筛选器可视化界面
...
from scanner_engine import (
    CONFIG, fetch_etf_list, fetch_etf_kline, fetch_index_kline,
    ...
    check_canslim_m, run_scan_engine, run_backtest_engine,
...
def render_sidebar():
    ...
    # ---- CAN SLIM 开关 ----
    st.sidebar.subheader("  CAN SLIM 条件")
    n_enabled = st.sidebar.checkbox("N: 新高/板块强", CONFIG["N_ENABLED"])
    s_enabled = st.sidebar.checkbox("S: 供需关系", CONFIG["S_ENABLED"])
    l_enabled = st.sidebar.checkbox("L: 领涨领跌", CONFIG["L_ENABLED"])
    m_enabled = st.sidebar.checkbox("M: 市场方向", CONFIG["M_ENABLED"])
    c_enabled = st.sidebar.checkbox("C: 当季盈利 [占位]", CONFIG["C_ENABLED"])
    a_enabled = st.sidebar.checkbox("A: 年度盈利 [占位]", CONFIG["A_ENABLED"])
    i_enabled = st.sidebar.checkbox("I: 机构认同 [占位]", CONFIG["I_ENABLED"])

    if c_enabled:
        st.sidebar.warning("C条件需要补充季度盈利数据")
    if a_enabled:
        st.sidebar.warning("A条件需要补充年度盈利数据")
    if i_enabled:
        st.sidebar.warning("I条件需要补充机构持仓数据")

    st.sidebar.markdown("---")

    # ---- 技术面开关 ----
    st.sidebar.subheader("  技术面开关")
    ...
    # ---- N条件参数 ----
    n_new_high_months = st.sidebar.slider("N: 近N月新高", 1, 6, CONFIG["N_NEW_HIGH_MONTHS"])
    n_sector_rank = st.sidebar.slider("N: 板块排名前%", 10, 50, int(CONFIG["N_SECTOR_RANK_PCT"] * 100))

    # ---- S条件参数 ----
    s_scale_min = st.sidebar.number_input("S: 最小规模(亿)", 5, 100, CONFIG["S_SCALE_MIN"])
    s_scale_max = st.sidebar.number_input("S: 最大规模(亿)", 50, 500, CONFIG["S_SCALE_MAX"])
    ...
    config_override = {
        ...
        "N_ENABLED": n_enabled, "S_ENABLED": s_enabled, "L_ENABLED": l_enabled,
        "M_ENABLED": m_enabled, "C_ENABLED": c_enabled, "A_ENABLED": a_enabled,
        "I_ENABLED": i_enabled,
        ...
        "N_NEW_HIGH_MONTHS": n_new_high_months, "N_SECTOR_RANK_PCT": n_sector_rank / 100,
        "S_SCALE_MIN": s_scale_min, "S_SCALE_MAX": s_scale_max,
    }
...
def plot_kline_with_indicators(df: pd.DataFrame, title: str = "K线图",
    ...
    fig.update_layout(
        title=title,
        height=600 + (subplot_count - 1) * 100,
...
def plot_backtest_results(trades_df: pd.DataFrame) -> go.Figure:
    ...
    for i, (col, title) in enumerate([("ret_1周", "1周"), ("ret_2周", "2周"), ("ret_4周", "4周")]):
        if col in trades_df.columns:
            ...
            if len(valid) > 0:
                row = 1 if i < 2 else 2
                c = 1 if i % 2 == 0 else 2
                colors = ['#ef5350' if v >= 0 else '#26a69a' for v in valid]
    ...
    fig.update_layout(
        ...
        showlegend=False, margin=dict(l=50, r=20, t=50, b=30),
    )
    return fig
...
def tab_results(config_override):
    ...
    with col2:
        st.caption("点击运行完整CAN SLIM + 技术面筛选流程")
    ...
    if "scan_result" in st.session_state:
        ...
        c3.metric("技术面通过", stats.get("canslim_pass", 0))
        c4.metric("最终入选", stats.get("final_pass", 0))
        c5.metric("大盘方向", "多头" if m_result.get("pass", False) else "空头",
                  delta=m_result.get("detail", ""))
        ...
        st.markdown("---")
        st.subheader("  大盘趋势报告")
        if m_result.get("pass", False):
            st.success(f"  {m_result.get('detail', '')}")
        else:
            st.warning(f"  {m_result.get('detail', '')}")
...
def tab_market():
    ...
    if st.button("加载大盘数据", type="primary"):
        with st.spinner("正在获取沪深300和创业板指数据..."):
            try:
                csi300_w = fetch_index_kline(CONFIG["CSI300_CODE"], period="周")
                chinext_w = fetch_index_kline(CONFIG["CHINEXT_CODE"], period="周")
                csi300_m = fetch_index_kline(CONFIG["CSI300_CODE"], period="月")
                chinext_m = fetch_index_kline(CONFIG["CHINEXT_CODE"], period="月")

                st.session_state["csi300_weekly"] = csi300_w
                st.session_state["chinext_weekly"] = chinext_w
                st.session_state["csi300_monthly"] = csi300_m
                st.session_state["chinext_monthly"] = chinext_m

                st.success("数据加载完成")
            except Exception as e:
                st.error(f"获取失败: {e}")

    if "csi300_weekly" in st.session_state:
        csi300_w = st.session_state["csi300_weekly"]
        chinext_w = st.session_state["chinext_weekly"]

        # M条件判断
        m_result = check_canslim_m(csi300_w, chinext_w)

        c1, c2 = st.columns(2)
        with c1:
            if m_result.get("pass", False):
                st.success(f"  大盘方向：**偏多**\n\n{m_result['detail']}")
            else:
                st.warning(f"  大盘方向：**偏空**\n\n{m_result['detail']}")

        with c2:
            if len(csi300_w) >= 12:
                c = csi300_w["close"]
                ret = (c.iloc[-1] / c.iloc[-12] - 1) * 100
                st.metric("沪深300近3月涨幅", f"{ret:.2f}%", delta=f"{ret:.1f}%")

        st.markdown("---")

        # 沪深300图表
        st.subheader("  沪深300 周K线")
        if len(csi300_w) > 0:
            fig = plot_kline_with_indicators(csi300_w, title="沪深300 周K线")
            st.plotly_chart(fig, use_container_width=True)

        # 创业板指图表
        st.subheader("  创业板指 周K线")
        if len(chinext_w) > 0:
            fig = plot_kline_with_indicators(chinext_w, title="创业板指 周K线")
            st.plotly_chart(fig, use_container_width=True)

        # 板块涨幅排名
        st.subheader("  板块涨幅排名")
        if "scan_result" in st.session_state:
            sector_1m = st.session_state["scan_result"].get("sector_1m_returns", {})
            sector_3m = st.session_state["scan_result"].get("sector_3m_returns", {})
            if sector_1m:
                sector_df = pd.DataFrame([
                    {"板块": k, "近1月涨幅%": round(v, 2),
                     "近3月涨幅%": round(sector_3m.get(k, 0), 2)}
                    for k, v in sorted(sector_1m.items(), key=lambda x: x[1], reverse=True)
                ])
                st.dataframe(sector_df, use_container_width=True)
        else:
            st.info("请先在「筛选结果」页运行筛选，即可查看板块涨幅排名")
...
def tab_backtest(config_override):
    st.header("  回测报告")
    st.caption("回测仅基于技术面信号（MACD金叉+放量+RSI），不含基本面筛选")
...
# ==================================================================================
def main():
    st.title("  大A改良版 CAN SLIM ETF 右侧交易筛选器")
    st.caption("多周期共振 | 右侧交易 | 纯本地运行")
    ...
    # 页脚
    st.markdown("---")
    st.caption("CAN SLIM ETF筛选器 v1.0 | 数据源: akshare | 指标参数与同花顺对齐")
